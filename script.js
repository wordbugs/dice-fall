...
const app = createApp({
  data() {
    return {
      ...
      intendedSettings: {
        ...
        homeDiceType: {
          // amount is not defined here; it's this.intendedPlayers
          // and it's cannot be affected by board size
          setSuit: false,
          suitIsInherited: true,
          faces: [{ value: 4, modifiers: new Set(["lookout"]) }],
        },
        diceTypes: [
          {
            amount: 4,
  ...
  computed: {
    ...
    deck() {
      ...
      for (let i = deck.length; i > this.size - n; i--) {
        deck.pop();
      }
    ...
    deckSize() {
      let count = this.intendedPlayers.length;
      for (const tile of this.intendedSettings.diceTypes) {
        count += tile.amount;
      }
      return count;
...
app.component("input-presets", {
    ...
    return {
      presets: [
        ...
        },
        {
          ...
          columns: 4,
          rows: 4,
          homeDiceType: { faces: [{ value: 4, modifiers: ["lookout"] }] },
          diceTypes: [
        ...
        },
        {
          ...
          homeDiceType: {
            setSuit: true,
            faces: [{ value: 4, modifiers: ["lookout"] }],
          },
          diceTypes: [
            {
              amount: 64,
  ...
  methods: {
    ...
    pickPreset(preset) {
      ...
      this.fixDiceType(settings.homeDiceType, true);

      settings.homeDiceType = preset.homeDiceType;
    ...
    fixDiceType(diceType, home = false) {
      if (
      ...
      if (home) {
        diceType.faces[0].modifiers.add("home");
      } else {
